...
def read_segments(filename, bottom, top):
    segments = []
    with open(filename) as f:
        for line in f:
            x1, y1, x2, y2 = map(float, line.split())
            if y1 == bottom.p.y and y2 == bottom.q.y or x1==x2 and x1==bottom.p.x or x1==x2 and x1==bottom.q.x:
                pass
            elif y1==top.p.y and y2==top.q.y or x1==x2 and x1==top.p.x or x1==x2 and x1==top.q.x:
                pass
            else:
                p1 = Point(x1, y1)
                p2 = Point(x2, y2)
...
def main():
    #Bounding box
    xmin, xmax = 636301.0, 641001.0
    ymin, ymax = 219001.0, 222751.0
    #we declare the segments we want to insert
    segments = read_segments("segments.txt", Segment(Point(xmin,ymin), Point(xmax,ymin)), Segment(Point(xmin,ymax), Point(xmax,ymax)))
